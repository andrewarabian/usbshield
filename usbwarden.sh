...
_USBGUARD_ELEVATED=${_USBGUARD_ELEVATED:-0}
if [[ $EUID -ne 0 ]]; then
    [[ "$_USBGUARD_ELEVATED" == "1" ]] && { echo "  ERROR: Elevation failed. Recursive sudo may be disabled." >&2; exit 1; }
    _USBGUARD_ELEVATED=1 exec sudo -E "$0" "$@"
fi
